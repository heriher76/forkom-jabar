                    ...
                    <div class="table-responsive">
                        <table class="table table-bordered table-striped table-hover js-basic-example dataTable">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{ $workProgram->title }}</td>
                                    <td>{{ str_limit(strip_tags($workProgram->description), $limit = 90, $end = '...') }}</td>
                                    <td>{{ $workProgram->icon }}</td>
                                    <td>
                                        <a href="{{ url('admin/services/'.$workProgram->id.'/edit') }}" class="btn btn-success btn-xs waves-effect">Edit</a>
                                        <form action="{{ url('admin/services/'.$workProgram->id) }}" method="POST" style="display: inline;">
                                            {{ csrf_field() }}
                                            {{ method_field('delete') }}
